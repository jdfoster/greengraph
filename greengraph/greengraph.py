...
class Greengraph(object):
    def __init__(self, start, end):
        if not (isinstance(start, str) & isinstance(end, str)):
            raise TypeError('Geographical locations should be a strings')
        if (len(start) < 2) | (len(end) < 2):
            raise TypeError('Locations should string with two characters or more')
    ...
    def location_sequence(self, start, end,  steps):
        ...
        if not ((len(start) == 2) & (len(end) == 2)):
            raise TypeError('Coordinate iterable container containing two float values')
        if not(bool_iter_float(start) & bool_iter_float(end)):
            raise TypeError('Coordinate within iterable container need to be floats')
